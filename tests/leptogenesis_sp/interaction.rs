    ...
    solver.add_interaction(move |mut s, n, ref c| {
        ...
        let gamma_tilde = {
            ...
            m2 *= (c.model.m_n[0].powi(2) - c.model.m_h.powi(2))
                * bessel::k_1_on_k_2(c.model.m_n[0] * c.beta);
            m2 /= c.hubble_rate * c.beta * 16.0 * PI_1 * c.model.m_n[0];
            m2
        };
...
pub fn n_el_ql_qr(solver: &mut NumberDensitySolver<VanillaLeptogenesisModel>) {
    ...
    solver.add_interaction(move |mut s, n, ref c| {
        let gamma = {
            ...
            m2_prefactor * integrate_st(m2_st, c.beta, c.model.m_n[0], 0.0, 0.0, 0.0)
