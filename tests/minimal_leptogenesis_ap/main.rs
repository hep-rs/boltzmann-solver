...
fn minimal_leptogenesis_ap() {
    ...
    assert!(1e-10 < *sol[0].as_abs() && *sol[0].as_abs() < 1e-5);
    assert!(sol[1] < 1e-20);
}
...
fn interaction_n_el_h(solver: &mut NumberDensitySolver<VanillaLeptogenesisModel>) {
    ...
    solver.add_interaction(move |mut s, n, ref c| {
        ...
        let gamma_tilde = {
            let mut m2 = Float::with_val(c.working_precision, 0.0);
            for b in 0..3 {
        ...
        // let _decay = gamma_tilde.clone() * &n[1];
        let inverse_decay = gamma_tilde.clone() * c.eq_n[1];
        let net_decay = gamma_tilde.clone() * (n[1].clone() - c.eq_n[1]);

        let mut dbl = net_decay.clone() * c.model.epsilon;
        dbl -= &n[0] * inverse_decay;
        s[0] += &dbl;
        let dn1 = -net_decay;
        s[1] += &dn1;
...
fn interaction_n_el_ql_qr(solver: &mut NumberDensitySolver<VanillaLeptogenesisModel>) {
    ...
    solver.add_interaction(move |mut s, n, ref c| {
        ...
        let forward = gamma.clone() * checked_div(n[1].to_f64(), c.eq_n[1]);
        let net_forward = gamma.clone() * (checked_div(n[1].to_f64(), c.eq_n[1]) - 1.0);
        let backward = gamma;
